...
export function AddFactForm({ onAdd }: AddFactFormProps) {
  ...
  return (
    <Card className="border-2 border-dashed border-primary/20 bg-primary/5 hover:bg-primary/10 transition-colors mb-8 shadow-sm">
      <CardContent className="p-4">
        {!isExpanded ? (
          <Button
            variant="ghost"
            className="w-full h-12 flex items-center justify-center gap-2 text-muted-foreground hover:text-primary"
          ...
          <form onSubmit={handleSubmit} className="space-y-4 animate-in slide-in-from-top-2">
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button type="submit" disabled={!content.trim()} className="bg-primary hover:bg-primary/90">
                Add Fact
              </Button>
            </div>
          </form>
